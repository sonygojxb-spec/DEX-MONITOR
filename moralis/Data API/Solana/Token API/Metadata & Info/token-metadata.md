...
````yaml /openapi-files/data-api/solana-api.json GET /token/{network}/{address}/metadata
openapi: 3.0.0
info:
...
paths:
  /token/{network}/{address}/metadata:
    get:
      ...
      description: >-
        Get the global token metadata for a given network and contract (mint,
        standard, name, symbol, metaplex).
